...
async function loadEnvFile() {
  const candidates = [new URL('../.env', import.meta.url), new URL('.env', `file://${process.cwd().replace(/\\/g, '/')}/`)];
...
function parseBnbJsonLd(payload) {
  ...
  const title = firstByKeyRegex(root, /title/i);
  const publisher = firstByKeyRegex(root, /publisher/i);
  const publishDate = firstByKeyRegex(root, /date|issued|publication/i);
  const publishPlace = firstByKeyRegex(root, /spatial|place/i);
  const pageCount = firstByKeyRegex(root, /extent|pagination|pages?/i);
  const edition = firstByKeyRegex(root, /edition/i);
  const languageCode = firstByKeyRegex(root, /language/i);
  const summary = firstByKeyRegex(root, /description|summary|abstract/i);
  const seriesTitle = firstByKeyRegex(root, /series|ispartof/i);
  const seriesVolume = firstByKeyRegex(root, /partnumber|volume|numbering/i);
  const authors = namesFromRel(/creator|author/i);
...
export async function handleLookupDnb(req, res) {
  const body = parseBody(req);
  const inputValues = Array.isArray(body.isbns) ? body.isbns : String(body.isbns || '').split(/\r?\n/);
